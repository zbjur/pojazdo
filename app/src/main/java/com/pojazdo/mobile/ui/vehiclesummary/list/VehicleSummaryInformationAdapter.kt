...
import com.pojazdo.architecture.ui.summary.list.avarageprices.VehicleAveragePriceSection
...
import com.pojazdo.architecture.ui.summary.list.map.VehicleMapHolder
import com.pojazdo.architecture.ui.summary.list.map.VehicleMapSection
import com.pojazdo.architecture.ui.summary.list.rating.UserCommentsHolder
import com.pojazdo.architecture.ui.summary.list.rating.UserComments
import com.pojazdo.architecture.ui.summary.list.sellerdetails.VehiclePersonalDataGroupHolder
import com.pojazdo.architecture.ui.summary.list.sellerdetails.VehiclePersonalDataSection
import com.pojazdo.architecture.ui.summary.list.timeline.VehicleTimeLine
import com.pojazdo.architecture.ui.summary.list.timeline.VehicleTimeLineHolder
import com.pojazdo.architecture.ui.summary.list.vehiclesummary.VehicleSummaryInfoHolder
import com.pojazdo.architecture.ui.summary.list.vehiclesummary.VehicleSummaryInfoModel
...
/*CHILD'S VIEW TYPES*//*
    ...
    override fun onBindGroupViewHolder(holder: GroupViewHolder?, flatPosition: Int, group: ExpandableGroup<*>?) {
        when (getItemViewType(flatPosition)) {
            VEHICLE_SUMMARY_INFO_GROUP_VIEW_TYPE ->
                (holder as VehicleSummaryInfoHolder).setVehicleSummaryInfo(group as VehicleSummaryInfoModel)
            VEHICLE_PERSONAL_DATA_GROUP_VIEW_TYPE ->
    ...
    override fun onCreateChildViewHolder(parent: ViewGroup?, viewType: Int): ChildViewHolder {
        return when (viewType) {
            ...
            VEHICLE_OVERALL_RATING_VIEW_TYPE ->
                UserCommentsHolder(LayoutInflater.from(parent?.context).inflate(R.layout.vehicle_overall_rating, parent, false))
            ...
            VEHICLE_USER_RATING_VIEW_TYPE ->
                UserCommentsHolder(LayoutInflater.from(parent?.context).inflate(R.layout.vehicle_user_comments_item, parent, false))
            VEHICLE_AD_VIEW_TYPE ->
                VehicleSaleOffersHolder(LayoutInflater.from(parent?.context).inflate(R.layout.vehicle_ad_parent_recycle, parent, false))
            VEHICLE_PRICE_OPTION_VIEW_TYPE ->
                VehicleAveragePriceHolder(LayoutInflater.from(parent?.context).inflate(R.layout.vehicle_price_item, parent, false))
    ...
    override fun onBindChildViewHolder(holder: ChildViewHolder?, flatPosition: Int, group: ExpandableGroup<*>?, childIndex: Int) {
        ...
        when (viewType) {
            ...
            VEHICLE_OVERALL_RATING_VIEW_TYPE ->
                (holder as UserCommentsHolder).setOverallRating(vehicleSubsection.subsection as UserComments)
            VEHICLE_RATING_VIEW_TYPE ->
            ...
            VEHICLE_USER_RATING_VIEW_TYPE ->
                (holder as UserCommentsHolder).setUserComments(vehicleSubsection.subsection as VehicleUserRatingSection)

            VEHICLE_AD_VIEW_TYPE -> {
                (holder as VehicleSaleOffersHolder).valueTitle?.text = "Simple text"
                ...
                holder.recycleView?.adapter = VehicleAdsAdapter(context, (vehicleSubsection.subsection as VehicleSaleOffersSection).vehicleOfferSections)
                holder.recycleView?.setRecycledViewPool(viewPool)

            }
            VEHICLE_PRICE_OPTION_VIEW_TYPE ->
                (holder as VehicleAveragePriceHolder).setPrices(vehicleSubsection.subsection as VehicleAveragePriceSection)
            VEHICLE_PERSONAL_DATA_CHILD_VIEW_TYPE ->
